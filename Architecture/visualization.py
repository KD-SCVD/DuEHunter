...
def setup_logging():
    """设置日志记录"""
    # 创建日志目录
    log_dir = "loges/visualization/"
    os.makedirs(log_dir, exist_ok=True)

    # 设置日志文件名
    log_filename = os.path.join(
        log_dir,
        f"None-{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.loge"
...
parser.add_argument('--train_file_path',default='../Existing Dataset/RE/train_dataset.json', type= str, help= 'CFG input path' )
parser.add_argument('--test_file_path',default='../Existing Dataset/RE/test_dataset_nofp.json', type= str, help= 'CFG input path' )
parser.add_argument('--epoches', default= 10, help= ' the epoches of training')
...
if __name__ == '__main__':
    ...
    # model process
    logging.info("初始化模型")
    model = Model(input_dim=256, perturbation="None").to(device)
    logging.info("模型初始化完成, noise = None")
